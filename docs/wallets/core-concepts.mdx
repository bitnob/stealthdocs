---
title: "Wallets core concepts"
sidebarTitle: "Core concepts"
description: "Vaults, wallets, policies, and approvals: the objects to model your Wallets integration around."
related:
  - /docs/wallets/authentication
  - /docs/wallets/exchange-integration-guide
---
These are the building blocks behind Bitnob Wallets. Understanding them will help you model your integration correctly and configure policies that reflect your actual security requirements.

## Custody model

Bitnob Wallets is non-custodial. You hold the assets, and Bitnob cannot move them. Private keys are generated inside the Bitnob Secure Enclave, a key management system built on AWS Nitro Enclaves, and never leave that hardware boundary.
...
- **Vault**: an HD seed container. A dedicated vault maps 1-to-1 with a single wallet per chain. A pool vault holds unlimited wallets, ideal for assigning one per customer. See [Vaults](/api-reference/wallets/vaults/list-vaults).
- **Wallet**: a logical account derived from a vault's seed, scoped to a specific chain. Holds multiple asset balances and owns all addresses derived from it. See [Wallets](/api-reference/wallets/wallets/list-wallets).
- **Address**: a blockchain address derived from the wallet's HD path. Each wallet gets one primary address per chain; additional addresses can be [derived on demand](/api-reference/wallets/wallets/create-address) and [listed](/api-reference/wallets/wallets/list-addresses).

## Vault types

Choose the vault type that matches your use case. The choice is irreversible, upgrading a dedicated vault to a pool vault cannot be undone. Existing vaults can be inspected with [List Vaults](/api-reference/wallets/vaults/list-vaults) and [Get Vault](/api-reference/wallets/vaults/get-vault).
...
The type is set on [Create Wallet](/api-reference/wallets/wallets/create-wallet) and can be reviewed with [List Wallets](/api-reference/wallets/wallets/list-wallets) or changed with [Update Wallet](/api-reference/wallets/wallets/update-wallet).
...
A gas station is a pre-funded, organisation-owned account whose sole job is to sponsor transaction fees on behalf of customer wallets: see [Gas Stations](/api-reference/wallets/gas-stations/list-gas-stations) and [Sponsor and Broadcast](/api-reference/wallets/gas-stations/sponsor-and-broadcast-external-tron-transaction).
...
Every outbound transaction moves through a defined set of statuses. Understanding these helps you design correct notification handlers and UI states. The calls involved are [Create Transaction](/api-reference/wallets/transactions/create-transaction), [Get Transaction](/api-reference/wallets/transactions/get-transaction), and [List Transactions](/api-reference/wallets/transactions/list-transactions).
